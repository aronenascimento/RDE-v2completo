...
const ActiveCampaignForm: React.FC<ActiveCampaignFormProps> = ({ onSuccess, targetLink }) => {
  ...
  useEffect(() => {
    // 1. Inject AC script
    const scriptContent = `
      window.cfields = {"2":"utmsource","4":"utmcontent","5":"utmmedium","6":"utmterm","7":"utmcampaign"};
      window._show_thank_you = function(id, message, trackcmp_url, email) {
          // NOTE: We rely on MutationObserver/Polling in React component to detect success, 
          // but the AC script must run its internal logic to display the success message.
          ...
          thank_you.focus();
          // Removed call to window._form_callback here to avoid reliance on AC's internal flow
      };
              ...
              if (elem.getAttribute('required') !== null) {
                  if (elem.type == 'radio' || (elem.type == 'checkbox' && /any/.test(elem.className))) {
                      ...
                      if (elem.multiple) {
                          ...
                          for (var i = 0; i < elem.options.length; i++) {
<dyad-write path="src/components/ActiveCampaignForm.tsx" description="Fixing TypeScript error by importing useState">
import React, { useEffect, useRef, useState } from 'react';
...
const ActiveCampaignForm: React.FC<ActiveCampaignFormProps> = ({ onSuccess, targetLink }) => {
  ...
  useEffect(() => {
      ...
      (function() {
          ...
          var initializePhoneInput = function(inputPhone) {
              ...
              window._load_script('https://cdn.jsdelivr.net/npm/intl-tel-input@25.2.0/build/js/intlTelInput.min.js', function() {
                  ...
                  iti = intlTelInput(inputPhone, {
                      ...
                      initialCountry: 'br', // Alterado para 'br'
                      // geoIpLookup removido para forçar 'br'
                  });
              }, false);
          }
          ...
          var form_submit = function(e) {
              ...
              if (validate_form()) {
                  ...
                  var submitButton = e.target.querySelector('#_form_${FORM_ID}_submit');
                  submitButton.disabled = true;
                  submitButton.classList.add('processing');
                          var serialized = _form_serialize(document.getElementById('_form_${FORM_ID}_')).replace(/%0A/g, '\\n');
                  ...
                  async function submitForm() {
                      ...
                      const response = await fetch('https://residentelitemarketing.activehosted.com/proc.php?jsonp=true', request);
                  ...
                  if (formSupportsPost) {
                      submitForm().then((data) => {
                          ...
                          _show_error("${FORM_ID}", "Desculpe, seu envio falhou. Tente novamente.");
                      });
                  } else {
    ...
    // Adicionando tratamento de erro para scripts bloqueados
    scriptElement.onerror = () => {
      console.error("Falha ao carregar script do ActiveCampaign. Provável AdBlock.");
      setScriptError(true);
    };
    
    formRef.current?.appendChild(scriptElement);

    // 2. Setup MutationObserver and Polling for success detection
    const formElement = formRef.current?.querySelector(`#_form_${FORM_ID}_`) as HTMLFormElement | null;
        ...
        if (mutation.type === 'childList') {
          mutation.addedNodes.forEach(node => {
            if (node instanceof HTMLElement) {
              const isThankYou = node.classList.contains('_form-thank-you') || node.querySelector('._form-thank-you');
              const containsObrigado = node.textContent?.includes('Obrigado');

              if (isThankYou || containsObrigado) {
                console.log('Sucesso detectado via MutationObserver');
                observer.disconnect();
                handleSuccess();
                return;
              }
    ...
    const pollingInterval = setInterval(() => {
      const thankYouElement = formElement.querySelector('._form-thank-you') as HTMLElement | null;
      const formContentElement = formElement.querySelector('._form-content') as HTMLElement | null;

      const isThankYouVisible = thankYouElement && thankYouElement.style.display !== 'none';
      const isFormHidden = formContentElement && formContentElement.style.display === 'none';

      if (isThankYouVisible || isFormHidden) {
        console.log('Sucesso detectado via polling');
        handleSuccess();
      }
    }, 500);
  ...
  const formHtml = `
    ...
    <form method="POST" action="https://residentelitemarketing.activehosted.com/proc.php" id="_form_${FORM_ID}_" class="_form _form_${FORM_ID} _inline-form  _dark" novalidate data-styles-version="5">
      ...
      <input type="hidden" name="or" value="a282cb76-5702-4ac3-a83a-f9aac030abac" />
      <div class="_form-content">
        <div class="_form_element _x33786469 _full_width " >
        ...
        <div class="_form_element _x88679662 _full_width " >
      ...
      <div class="_form_element _x67166779 _full_width " >
      ...
      <div class="_form_element _x94004816 _full_width " >
        <input type="hidden" name="field[2]" value="" />
      </div>
      <div class="_form_element _x63509884 _full_width " >
        <input type="hidden" name="field[4]" value="" />
      </div>
      <div class="_form_element _x81696847 _full_width " >
        <input type="hidden" name="field[5]" value="" />
      </div>
      <div class="_form_element _x73569747 _full_width " >
        <input type="hidden" name="field[6]" value="" />
      </div>
      <div class="_form_element _x53213055 _full_width " >
        <input type="hidden" name="field[7]" value="" />
      </div>
      <div class="_button-wrapper _full_width">
        <button id="_form_${FORM_ID}_submit" class="_submit" type="submit">
          QUERO ENTRAR
        </button>
      </div>
  ...
  // Se houver erro de script, exibe mensagem alternativa
  if (scriptError) {
    return (
      <div className="p-6 text-center">
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          <p className="text-red-800 text-sm">
            <strong>Problema de conexão:</strong> Parece que seu navegador está bloqueando scripts necessários para o formulário.
          </p>
          <p className="text-red-600 text-xs mt-2">
            Tente desativar temporariamente bloqueadores de anúncios ou pop-ups e recarregue a página.
          </p>
          <button 
...
// Declaração global para o TypeScript não reclamar do callback do ActiveCampaign
declare global {
  interface Window {
    _form_callback?: (id: number) => void;
  }
}